...
static const size_t VGA_MEMORY_SIZE = VGA_WIDTH * VGA_HEIGHT * 2; // x2 or char and colour
...
void terminal_initialize(void) {
	terminal_row = 0;
	terminal_column = 0;
	terminal_color = vga_colour(VGA_COLOR_GREEN, VGA_COLOR_BLACK);
	terminal_clear();
...
void terminal_clear(void)
{
	...
	uint16_t clear_char =  vga_char(' ', terminal_color);
	for (size_t i = 0; i < VGA_MEMORY_SIZE; i++)
	{
		terminal_buffer[i] = vga_char(' ', terminal_color);
	}
